...
export abstract class AbstractCommand<TRequest, TResponse> {
    ...
    public abstract authenticate(request: AuthenticateUserRequest): Promise<Token>;
    ...
    protected async apiCaller(wrappedRequest: WrappedRequest, retryCount: number): Promise<Token> {
        ...
        switch (wrappedRequest.verb) {
            ...
            case RequestVerbType.POST_FORM:
                ...
                for (const key in Object.keys(wrappedRequest.data)){
        ...
        throw new RequestFailureError(JSON.stringify({status_code: response.status, request: wrappedRequest, retryCount: retryCount}));
    }
}
